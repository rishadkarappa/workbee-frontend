...
// pages/user/ClientMessages.tsx  (replaces existing ClientMessages)
import { useEffect, useRef, useState, useCallback } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import { socketService } from '@/services/socket-service';
import { ChatService } from '@/services/chat-service';
...
export default function ClientMessages() {
  ...
  // Track which confirm requests have been responded to (by workId)
  ...
  const handleAcceptConfirm = async (workId: string) => {
    ...
    // Find the confirm request to get workTitle and workerName
    ...
    try {
      // Update work status to 'assigned' in backend
      await WorkService.updateWork(workId, { status: 'assigned' });

      // Send accepted socket event
      await socketService.confirmResponse({
        ...
        accepted:   true,
        userId:     userId!,
        workerName: parsed?.type === 'WORK_CONFIRM_REQUEST' ? parsed.workerName : 'Worker',
      });
