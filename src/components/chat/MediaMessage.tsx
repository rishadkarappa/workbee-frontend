...
interface MediaMessageProps {
  ...
  mediaUrl: string;
  isSent: boolean;
  /** Called once the image renders or video metadata loads — parent uses this to re-scroll */
  onLoaded?: () => void;
}

export function MediaMessage({ type, mediaUrl, isSent, onLoaded }: MediaMessageProps) {
  const [loaded, setLoaded] = useState(false);
  const [error,  setError]  = useState(false);
  ...
  const handleImageError = () => {
    setError(true);
    setLoaded(false);
    onLoaded?.(); // still scroll — placeholder has fixed height now
  };
  ...
  return (
    <div className="relative max-w-xs rounded-lg overflow-hidden">
      <video
        ...
        onError={() => { setError(true); onLoaded?.(); }}
      >
        <track kind="captions" />
        Your browser does not support video.
      </video>
